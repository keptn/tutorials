...
1. Use the Keptn CLI to deploy a version of the carts service, which contains an artificial **slowdown of 1 second** in each request.
    ...
    <!-- command -->
    ```
    keptn send event new-artifact --project=sockshop --service=carts --image=docker.io/keptnexamples/carts --tag=0.11.2 --sequence=artifact-delivery
    ```
...
To verify, the [Keptn's Bridge](https://keptn.sh/docs/0.8.x/reference/bridge/) shows the deployment of v0.11.2 and then the failed test in staging including the roll-back.
...
1. Use the Keptn CLI to send a new version of the *carts* artifact, which does **not** contain any slowdown:
    ...
    <!-- command -->
    ```
    keptn send event new-artifact --project=sockshop --service=carts --image=docker.io/keptnexamples/carts --tag=0.11.3 --sequence=artifact-delivery
    ```
