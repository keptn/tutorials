...
Activate the quality gates for the carts service. Therefore, navigate to the `examples/onboarding-carts` folder and upload the `slo-quality-gates.yaml` file using the [add-resource](https://keptn.sh/docs/0.12.x/reference/cli/commands/keptn_add-resource/) command:
...
To verify, the [Keptn's Bridge](https://keptn.sh/docs/0.12.x/reference/bridge/) shows the deployment of v0.13.2 and then the failed test in staging including the roll-back.
