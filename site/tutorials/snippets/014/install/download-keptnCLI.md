...
- Easiest option (works on Linux, Mac OS, Windows with Bash and WSL2):
    ...
    <!-- command -->
    ```
    curl -sL https://get.keptn.sh | KEPTN_VERSION=0.14.2 bash
    ```
...
-  Another option is to manually download the current release of the Keptn CLI:
  1. Download the version for your operating system and architecture from [Download CLI](https://github.com/keptn/keptn/releases/tag/0.14.2)
  2. Unpack the download
  3. Find the `keptn` binary (e.g., `keptn-0.14.2-amd64.exe`) in the unpacked directory and rename it to `keptn`
